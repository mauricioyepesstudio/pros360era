...
**Everything above except Milestone 01/03/04A/04B (below) is still design-only.** The three Milestone 04 docs describe the full target architecture; Milestone 04A and 04B (below) implement a deliberately narrow, live slice of it (`opportunities`/`consent_receipts` + 7 RPCs total, `BUSINESS_MARKETING`/`FL` only, lifecycle through `ROUTED → CONTACTED → COMPLETED`/`DECLINED`) — the rest (`platform_events`, `SHARED_MAX_3`, sponsored placement, ML, regulated categories, any UI consuming the lifecycle RPCs) remains design-only exactly as documented. Treat the undelivered parts of this section like the rest of `docs/` before Phase 2 was implemented: a plan to build against, not a status report.
...
Not yet built: notifications of any kind, persisted `EXPIRED` rows, automatic rematching after decline/expiration, a freeform notes/message channel between member and professional, `platform_events`, any UI consuming these four new RPCs, rate limiting.

**Next exact milestone**: build the professional- and member-facing UI that actually calls these lifecycle RPCs (currently client-callable via `lib/opportunities/persistence.ts` but not wired into any page), independent of and not blocked by the still-unresolved Home visual cluster above.
