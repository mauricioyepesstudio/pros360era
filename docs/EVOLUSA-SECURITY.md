...
**C. No lifecycle writer exists yet for `CONTACTED`, `COMPLETED`, `DECLINED`, or `EXPIRED`.** `opportunities.status` can only ever reach `CREATED` or `ROUTED` today. This is the natural scope of Milestone 04B (see `docs/CURRENT-STATE.md`), not a defect in 04A — 04A's job was proving consent-gated routing, not the full lifecycle.
