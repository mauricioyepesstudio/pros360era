...
- **Resolved by Milestone 04D:** `/conexiones/nueva` now calls `create_qualified_opportunity` and `consent_and_route_opportunity`; both rematch/start-over links route there. See [EVOLUSA-OPPORTUNITY-START-FLOW.md](./EVOLUSA-OPPORTUNITY-START-FLOW.md).
- **Resolved by Milestone 04E (code-complete, migration not yet applied):** the member card and pre-consent step use `get_my_opportunity_professionals()` to obtain an allowlisted public professional summary for member-owned opportunities. The public view still does not expose its internal `id`; see [EVOLUSA-MATCHED-PROFESSIONAL-REVEAL.md](./EVOLUSA-MATCHED-PROFESSIONAL-REVEAL.md).
