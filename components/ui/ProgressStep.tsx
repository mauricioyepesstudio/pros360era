...
export default function ProgressStep({ status, title, description, children }: ProgressStepProps) {
  ...
  return <li className="flex gap-4"><span className={cn("mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full", status === "complete" && "bg-emerald-50 text-[var(--success)]", status === "current" && "bg-amber-50 text-[var(--warning)]", status === "upcoming" && "bg-slate-100 text-slate-500")}><Icon aria-hidden="true" size={17} /></span><div><p className="font-semibold text-[var(--brand-navy)]">{title}</p>{description && <p className="mt-1 text-sm leading-6 text-[var(--muted)]">{description}</p>}{children}</div></li>;
}
