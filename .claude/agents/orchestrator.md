...
- `revenue-analyst` — read-only health check on the real $25-per-connection Stripe fee mechanism (webhook-gated `mark_opportunity_contacted_paid`). Distinct from `finance-analyst` (which designs pricing/monetization models) — this one only reports on the mechanism that already exists. Never edits payment code, never touches Stripe, and never recommends a Sandbox→Live switch — that stays the owner's explicit call.
